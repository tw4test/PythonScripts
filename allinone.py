...
class UIStateManager:
    ...
    def set_state(self, new_state):
        ...
        with self.state_lock:
            ...
            print(f"[狀態變更] {old_state} -> {new_state}")
    ...
    try:
        ...
        for column in columns_to_add:
            try:
                ...
                print(f"[修復] 已添加列: {column}")
            except sqlite3.OperationalError as e:
                if "duplicate column name" in str(e).lower():
                    print(f"[跳過] 列 {column} 已存在")
                else:
                    print(f"[錯誤] 添加列 {column} 失敗: {e}")
...
def init_db(db_path=DB_PATH):
    ...
    try:
        ...
        if not existing_columns:
            ...
            """)
            print("[數據庫] 創建新的 files 表")
        else:
            print(f"[數據庫] 現有列: {existing_columns}")
            ...
            for column, definition in columns_to_add.items():
                if column not in existing_columns:
                    try:
                        ...
                        print(f"[數據庫升級] 添加列: {column}")
                    except sqlite3.OperationalError as e:
                        print(f"[警告] 添加列 {column} 失敗: {e}")
        
        conn.commit()
        
    except Exception as e:
        print(f"[數據庫錯誤] files 表處理失敗: {e}")
        raise
...
def scan_and_add_files(conn, source_root):
    ...
    stats = {'new_files': 0, 'updated_files': 0, 'duplicate_files': 0, 'error_files': 0}
    cur = conn.cursor()
    ...
    if RICH_AVAILABLE:
        with Progress(
            ...
            task = progress.add_task("掃描文件", total=len(all_files), current_file="準備中...")
            ...
            for i, file_info in enumerate(all_files):
                ...
                # 處理文件邏輯（簡化版）
                try:
                    cur.execute("SELECT id, status FROM files WHERE path=?", (file_info['full_path'],))
                    ...
                    if not existing:
                        ...
                        # 更新現有文件為待處理
                        cur.execute("UPDATE files SET status='pending' WHERE path=?", (file_info['full_path'],))
    ...
    # 只顯示一行總結
    console.print(f"[bold green]📁 掃描完成: {stats['new_files']} 新增, {stats['updated_files']} 更新, {stats['duplicate_files']} 重複[/bold green]")
...
class DynamicBatchManager:
    ...
    def start_new_batch(self):
        ...
        print(f"[動態批次] 開始批次: {self.current_batch_id}")
    ...
    def get_next_file_batch(self, max_files=None, max_size_gb=None):
        ...
        if selected_files:
            try:
                ...
                print(f"[記錄錯誤] 無法記錄批次歷史: {e}")
        
        print(f"[動態批次] 選擇 {len(selected_files)} 個文件，總大小 {current_size/1024/1024:.1f}MB")
    ...
    def complete_batch(self, batch_status='completed'):
        ...
        try:
            ...
            print(f"[批次完成] {self.current_batch_id}: {self.successful_pushes}/{total_files} ({success_rate:.1f}%)")
            
        except Exception as e:
            print(f"[批次記錄錯誤] 無法更新批次狀態: {e}")
        finally:
            # 重置狀態
            self.current_batch_id = None
            self.batch_files = []
...
class TrueDynamicBatchManager:
    ...
    def get_next_virtual_batch(self, max_files=None, max_size_gb=None):
        ...
        print(f"[虚拟批次] 内存中组建 {len(selected_files)} 个文件，{current_size/1024/1024:.1f}MB")
        return selected_files
    ...
    def commit_batch_to_database(self):
        ...
        print(f"[数据库提交] 一次性更新 {success_count} 个文件状态")
...
def move_remote_folder_safe(src_folder, dst_folder):
    ...
    try:
        ...
        print(f"[搬移成功] {src_folder} -> {dst_folder}")
        return True
        
    except Exception as e:
        print(f"[搬移失敗] {src_folder} -> {dst_folder}: {e}")
        return False

def cleanup_camera_folder(camera_folder):
    """清理Camera目錄中的批次資料夾"""
    try:
        print(f"[清理] 刪除Camera資料夾: {camera_folder}")
        adb_remove_remote_folder(camera_folder)
        
        # 觸發媒體掃描
        adb_trigger_media_scan(CAMERA_ROOT)
        print(f"[清理成功] {camera_folder}")
        
    except Exception as e:
        print(f"[清理失敗] {camera_folder}: {e}")
...
def wait_for_backup_complete():
    ...
    if params.get('quick_backup_detection', True):
        ...
        if current_batch_size < 100:
            ...
            print(f"[優化] 小批次 ({current_batch_size} 文件)，縮短等待時間至 {required_stable} 秒")
        elif current_batch_size < 500:
            required_stable = min(45, required_stable)
            print(f"[優化] 中等批次 ({current_batch_size} 文件)，調整等待時間至 {required_stable} 秒")
        else:
            print(f"[標準] 大批次 ({current_batch_size} 文件)，使用標準等待時間 {required_stable} 秒")
                    ...
                    with batch_processing_lock:
                        if not batch_in_process:
                            ...
                            if file_batch:
                                ...
                                try:
                                    console.print(f"[bold cyan]📦 處理批次 {total_processed_batches + 1}: {len(file_batch)} 個文件[/bold cyan]")
                                    ...
                                    if success_count > 0:
                                        ...
                                        if move_remote_folder_safe(remote_temp_folder, camera_folder):
                                            mark_pushed_files_completed(conn, file_batch)
                                            
                                            console.print("[yellow]⏳ 等待 Google Photos 備份完成...[/yellow]")
                                            ...
                                            console.print(f"[green]✓ 批次 {total_processed_batches} 完成[/green]")
                                            
                                            #if enhanced_batch_completion_check(conn, batch_manager, total_processed_batches):
                                            #    break
                                        else:
                                            console.print("[red]✗ 批次搬移失敗[/red]")
                    ...
                    with batch_processing_lock:
                        if not batch_in_process:
                            ...
                            if virtual_batch:
                                ...
                                try:
                                    console.print(f"[bold cyan]📦 处理虚拟批次: {len(virtual_batch)} 个文件[/bold cyan]")
                                    ...
                                    if success_count > 0:
                                        ...
                                        if move_remote_folder_safe(remote_temp_folder, camera_folder):
                                            ...
                                            # 等待备份完成
                                            console.print("[yellow]⏳ 等待 Google Photos 备份完成...[/yellow]")
                                            wait_for_backup_complete()
                                            ...
                                            console.print(f"[green]✓ 虚拟批次 {total_processed_batches} 完成，已提交 {committed_count} 个文件[/green]")
                                        else:
                                            console.print("[red]✗ 批次搬移失败[/red]")
                                    else:
                                ...
                                if check_all_files_processed_with_retry(conn):
                                    console.print(f"[bold green]🎉 所有文件处理完成！总共处理 {total_processed_batches} 个虚拟批次[/bold green]")
                                    show_completion_notification(total_processed_batches)
                                    break
                                else:
                                    time.sleep(1)
...
if __name__ == "__main__":
    ...
    # 自動啟動CPU監控
    print("[系統啟動] 正在啟動CPU監控...")
    auto_start_cpu_monitoring()
    print("[系統啟動] CPU監控已啟動")
    print("[系統提示] 點擊'開始傳輸'按鈕開始動態批次處理")
    print("[系統提示] UI狀態管理已啟用 - 防止重複操作")
    print("[系統說明] 動態批次管理 - 真正的斷點續傳功能")
